/**
 * "My Issues" push screen — moved out of the board tab so `(tabs)/board`
 * can host the workspace dashboard / project board. Three scopes —
 * assigned / created / agents — mirroring web's
 * `packages/views/my-issues/components/my-issues-page.tsx:48-65`.
 *
 * Header chrome (back + title) comes from the parent Stack
 * (`[workspace]/_layout.tsx`). Filter affordance stays in <ScopeToolbar>.
 *
 * Issues are grouped by status using SectionList in `BOARD_STATUSES` order;
 * empty status sections are filtered out. Filter state lives in
 * `useMyIssuesViewStore` and is cleared on workspace change via
