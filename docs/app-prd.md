...
- 区块标题：**工作成果**；副文案：`过去 24 小时 · N 份`；右侧：**完整报告 ›** → `/{slug}/reports`（量化报告下沉页，见下）。
- 最多 **2–3 条**竖向成果卡（收敛占屏；不做日/周/月胶囊、三格大数、分布条、sparkline）。
- 单卡结构：**上**头像 + 代号 + 职能；**下**类型 chip + 时段（昨夜 / 今早 / 午后 / 刚才）+ 标题/摘要 1–2 行。
- 类型枚举（先少后多）：`分析结论` | `日报摘要` | `新闻摘要` | `事项交付`。
- 点击卡片 → 有关联 `issue_id` 则 push `issue/[id]`；否则 mock 详情 / 占位（字段预留 `href`）。
- 空态：`过去 24 小时暂无产出` + 「派单或开启定时任务后会出现在这里」。
- 本期数据为 **B 类 mock**（`data/mocks/outcomes.ts` + `USE_MOCK_OUTCOMES` + 「示例」徽标），**禁止**在本卡 mock 完成数 / 时长 / Tokens。
...
- [ ] **工作成果**区块为员工产出流（上员工、下摘要），无日/周/月大指标卡；占屏明显低于旧报告卡。
- [ ] 工作成果 mock 带「示例」徽标，与 `USE_MOCK_OUTCOMES` 同源；不出现可被误判为真实的完成数 / Tokens。
...
| 屏 | 首屏请求 | 数量 | 说明 |
|---|---|---|---|
| 首页 | `issues(assigned 待办)`、`agent-task-snapshot`、`agents`、`briefs(mock)`、`outcomes(mock)` | **3 网络 + 2 本地** | 工作成果 / 简报为本地 mock；量化报告改在 `reports` 页按需拉 workspace issues |
...
| # | 内容 | 后端依赖 |
|---|---|---|
| 1 | **工作成果**首页区块（24h 员工产出流）+ `data/mocks/outcomes.ts` + `USE_MOCK_OUTCOMES` | 无 |
...
**验收关口**：工作成果为产出流且带示例徽标；报告页缺失指标显示 `——` 不显示 0；简报首页区块与详情页可用且带「示例数据」徽标。
...
| 版本 | 日期 | 变更 |
|---|---|---|
| v1.7 | 2026-08-17 | 首页 §4.4「数据分析报告」改为 **工作成果（过去 24h 员工产出流）**；日/周/月 A 类指标下沉 `/{slug}/reports`；与行业简报边界写清；首屏预算改为 3 网络 + 2 本地 mock |
